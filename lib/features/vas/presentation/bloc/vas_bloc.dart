...
class VasBloc extends Bloc<VasEvent, VasState> {
  ...
  VasBloc() : super(VasInitial()) {
    ...
    on<VasPurchaseTvCable>(_onPurchaseTvCable);
    on<VasReset>((_, emit) => emit(VasInitial()));
  }

  Future<void> _onLoadCatalog(VasLoadCatalog event, Emitter<VasState> emit) async {
    emit(VasCatalogLoading());
    final data = await _repo.getCatalog();
    if (data != null) {
      emit(VasCatalogLoaded(
        airtimeNetworks: _toList(data['airtime_networks']),
        dataPlans: _toList(data['data_plans']),
        electricityDiscos: _toList(data['electricity_discos']),
        bettingProviders: _toList(data['betting_providers']),
      ));
    } else {
  ...
  Future<void> _onPayElectricity(VasPayElectricity event, Emitter<VasState> emit) async {
    ...
    try {
      ...
      if (result != null) {
        ...
        emit(VasSuccess(message: token != null ? 'Payment successful. Your token is ready.' : 'Electricity payment successful.', reference: result['reference'] as String?, token: token, data: result));
      } else {
      ...
      await Future.delayed(const Duration(seconds: 3));
      emit(VasReversal(message: 'All utility routes failed. ₦${(event.amountKobo / 100).toStringAsFixed(0)} safely reversed to your wallet.', reference: 'REV-${DateTime.now().millisecondsSinceEpoch}'));
    } catch (_) {
      emit(const VasFailure(message: 'Electricity payment failed. Please try again.'));
    }
  }
  ...
  Future<void> _onFundBetting(VasFundBetting event, Emitter<VasState> emit) async {
    ...
    try {
      ...
      await Future.delayed(const Duration(seconds: 3));
      emit(VasReversal(message: 'All funding routes failed. ₦${(event.amountKobo / 100).toStringAsFixed(0)} safely reversed to your wallet.', reference: 'REV-${DateTime.now().millisecondsSinceEpoch}'));
    } catch (_) {
      emit(const VasFailure(message: 'Betting wallet funding failed. Please try again.'));
    }
  }

  Future<void> _onPurchaseTvCable(VasPurchaseTvCable event, Emitter<VasState> emit) async {
    emit(VasProcessing());
    try {
      final result = await _repo.payTvCable(
        smartCardNumber: event.smartCardNumber, provider: event.provider, planCode: event.planCode,
      ).timeout(const Duration(seconds: 45));
      if (result != null) {
        emit(VasSuccess(message: '${event.provider} subscription activated.', reference: result['reference'] as String?, data: result));
      } else {
        emit(const VasFailure(message: 'TV subscription failed. Please try again.'));
      }
    } on TimeoutException {
      emit(const VasReconciling(message: 'Provider timeout. Rerouting safely...'));
      await Future.delayed(const Duration(seconds: 3));
      emit(VasReversal(message: 'All routes failed. Funds safely reversed to your wallet.', reference: 'REV-${DateTime.now().millisecondsSinceEpoch}'));
    } catch (_) {
      emit(const VasFailure(message: 'TV subscription failed. Please try again.'));
